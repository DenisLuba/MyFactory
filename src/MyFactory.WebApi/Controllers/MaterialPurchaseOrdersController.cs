...
public class MaterialPurchaseOrdersController : ControllerBase
{
    ...
    public async Task<IActionResult> Receive(Guid purchaseOrderId, [FromBody] ReceiveMaterialPurchaseOrderRequest req)
    {
        await _mediator.Send(new ReceiveMaterialPurchaseOrderCommand
        {
            ...
            Allocations = (req.Allocations ?? [])
                .Select(i => new ReceiveMaterialPurchaseOrderAllocation
                {
                    WarehouseId = i.WarehouseId,
                    ShippingCost = i.ShippingCost,
                    MateialItems = (i.MaterialItems ?? [])
                        .Select(a => new ReceiveMaterialPurchaseOrderItem()
                        {
                            MaterialId = a.MaterialId,
                            QtyPerPackage = a.QtyPerPackage,
    ...
    public async Task<IActionResult> GetBySupplier(Guid supplierId)
    {
        ...
        var response = result
            .Select(o => new SupplierPurchaseOrderListItemResponse(o.Id, o.PurchaseNumber, o.OrderDate, o.Status, o.ItemsCount, o.TotalAmount))
            .ToList();
        return Ok(response);
    }
