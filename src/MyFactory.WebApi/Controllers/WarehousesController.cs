...
using MyFactory.Application.Features.Warehouses.DeactivateWarehouse;
...
using MyFactory.Application.Features.Warehouses.RemoveProductFromWarehouse;
...
using MyFactory.Application.Features.Warehouses.UpdateWarehouseProductQty;
...
public class WarehousesController : ControllerBase
{
    ...
    public async Task<IActionResult> GetList(
        ...
        [FromQuery] string[]? warehouseTypes = null)
    { 
        var dtos = await _mediator.Send(new GetWarehousesQuery(includeInactive, GetWarehouseTypes(warehouseTypes)));
        var response = dtos
    ...
    private static WarehouseType[]? GetWarehouseTypes(IEnumerable<string>? types)
    {
        IEnumerable<WarehouseType?>? result = types is null || !types.Any()
            ? null
            : [.. types
                  .Select(GetWarehouseType)
                  .Where(w => w is not null && w.HasValue)
              ];
        return result is null || !result.Any() ? null : [.. result.Select(w => w!.Value)];
    }

    private static WarehouseType? GetWarehouseType(string type)
    => type switch
    {
        "Materials" => WarehouseType.Materials,
        "FinishedGoods" => WarehouseType.FinishedGoods,
        "Aux" => WarehouseType.Aux,
        _ => null
    };
    ...
    public async Task<IActionResult> GetStock(Guid id)
    {
        ...
        var response = dtos
            .Select(x => new WarehouseStockItemResponse(x.ItemId, x.Name, x.QtyPerPackage, x.PackageCount, x.TotalQty, x.UnitCode))
            .ToList();
        return Ok(response);
    }
    ...
    {
        await _mediator.Send(new AddMaterialToWarehouseCommand(id, req.MaterialId, req.QtyPerPackage, req.PackageCount));
        return Ok();
    }
    ...
    {
        await _mediator.Send(new UpdateWarehouseMaterialQtyCommand(id, materialId, req.QtyPerPackage, req.PackageCount));
        return Ok();
    }
    ...
    public async Task<IActionResult> TransferMaterials([FromBody] TransferMaterialsRequest req)
    {
        await _mediator.Send(new TransferMaterialsCommand(
            req.FromWarehouseId,
            req.ToWarehouseId,
            req.Items.Select(i => new TransferMaterialItemDto(i.MaterialId, i.QtyPerPackage, i.PackageCount)).ToList()));
        return Ok();
    }
    ...
    {
        await _mediator.Send(new AddProductToWarehouseCommand(id, req.ProductId, req.QtyPerPackage, req.PackageCount));
        return Ok();
    }
    ...
    {
        await _mediator.Send(new RemoveProductFromWarehouseCommand(id, productId));
        return Ok();
    }
    ...
    [Consumes("application/json")]
    [ProducesResponseType(StatusCodes.Status200OK)]
    [SwaggerRequestExample(typeof(UpdateWarehouseProductQtyRequest), typeof(UpdateWarehouseProductQtyRequestExample))]
    public async Task<IActionResult> UpdateProductQty(Guid id, Guid productId, [FromBody] UpdateWarehouseProductQtyRequest req)
    {
        await _mediator.Send(new UpdateWarehouseProductQtyCommand(id, productId, req.QtyPerPackage, req.PackageCount));
        return Ok();
    }
    ...
    public async Task<IActionResult> TransferProducts([FromBody] TransferProductsRequest req)
    {
        await _mediator.Send(new TransferProductsCommand(
            req.FromWarehouseId,
            req.ToWarehouseId,
            req.Items.Select(i => new TransferProductItemDto(i.ProductId, i.QtyPerPackage, i.PackageCount)).ToList()));
        return Ok();
    }
}
