...
- **Endpoints:**
  ...
  - `POST /{id}/complete-stage` — завершить этап. (204 No Content).
  ...
  - `GET /{id}/stages/{stage}` — сотрудники этапа. `ProductionStageAssignmentResponse` (200 OK).
  - `GET /{id}/stages/{stage}/available-employees` — доступные сотрудники этапа. `AvailableProductionStageEmployeeResponse` (200 OK).
  - `POST /{id}/stages/{stage}` — добавить сотрудника. `AddProductionStageEmployeeRequest` (204 No Content).
  - `PUT /{id}/stages/{stage}/employees/{assignmentId}` — обновить сотрудника. `UpdateProductionStageEmployeeRequest` (204 No Content).
  - `DELETE /{id}/stages/{stage}/employees/{assignmentId}` — удалить сотрудника. (204 No Content).
  - `POST /{id}/sewing-operations` — зарегистрировать факт пошива. `RegisterSewingOperationRequest` → `RegisterSewingOperationResponse` (201 Created).
