...
public sealed class RemoveRoleCommandHandler : IRequestHandler<RemoveRoleCommand>
{
    ...
    public async Task Handle(RemoveRoleCommand request, CancellationToken cancellationToken)
    {
        ...
        var usersWithRole = await _context.Users
            .AnyAsync(u => u.RoleId == request.RoleId, cancellationToken);
        ...
        if (usersWithRole)
        {
            ...
            throw new InvalidOperationException($"Cannot remove role with name {roleName} because it is assigned to one or more users.");
        }
