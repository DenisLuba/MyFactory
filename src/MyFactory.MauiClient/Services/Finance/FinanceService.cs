...
public sealed class FinanceService : IFinanceService
{
    ...
    public async Task<ListResponse<PayrollAccrualListItemResponse>?> GetPayrollAccrualsAsync(
        DateOnly from,
        DateOnly to,
        ...
        int take = 30)
    {
        ...
        if (sortDesc)
        {
            query["sortDesc"] = bool.TrueString;
        }

        query["skip"] = Math.Max(0, skip).ToString(CultureInfo.InvariantCulture);
        query["take"] = Math.Clamp(take, 1, 100).ToString(CultureInfo.InvariantCulture);

        builder.Query = query.ToString();
        return await _httpClient.GetFromJsonAsync<ListResponse<PayrollAccrualListItemResponse>>(builder.Uri);
    }

        public async Task<ListResponse<PayrollAccrualListItemResponse>?> GetListAsync(
        ...
        Guid? fromId = null)
    { 
        return await GetPayrollAccrualsAsync(
