...
{
    Task<IReadOnlyList<WarehouseListItemResponse>?> GetListAsync(bool includeInactive = false, string[]? warehouseTypes = null);
    ...
    Task RemoveProductAsync(Guid warehouseId, Guid productId);
    Task UpdateProductQtyAsync(Guid warehouseId, Guid productId, UpdateWarehouseProductQtyRequest request);
