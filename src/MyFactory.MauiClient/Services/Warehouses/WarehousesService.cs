...
public sealed class WarehousesService : IWarehousesService
{
    ...
    public async Task<IReadOnlyList<WarehouseListItemResponse>?> GetListAsync(bool includeInactive = false, string[]? warehouseTypes = null)
    {
        var query = new List<string>();

        if (includeInactive) 
            query.Add("includeInactive=true");
        if (warehouseTypes is not null)
        {
            var warehouses = string.Join("&",
                warehouseTypes
                .Where(t => !string.IsNullOrWhiteSpace(t))
                .Select(t => $"warehouseTypes={Uri.EscapeDataString(t.Trim())}"));
            query.Add(warehouses);
        }

        var path = query.Count != 0 ? "?" + string.Join("&", query) : string.Empty;

        return await _httpClient.GetFromJsonAsync<List<WarehouseListItemResponse>>($"api/warehouses{path}");
    }
	...
	public async Task RemoveProductAsync(Guid warehouseId, Guid productId)
	{
		var response = await _httpClient.DeleteAsync($"api/warehouses/{warehouseId}/products/{productId}");
        await response.EnsureSuccessWithProblemAsync();
	}

	public async Task UpdateProductQtyAsync(Guid warehouseId, Guid productId, UpdateWarehouseProductQtyRequest request)
	{
		var response = await _httpClient.PutAsJsonAsync($"api/warehouses/{warehouseId}/products/{productId}", request);
        await response.EnsureSuccessWithProblemAsync();
	}
}
