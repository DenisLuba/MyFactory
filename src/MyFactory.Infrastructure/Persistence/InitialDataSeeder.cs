...
public class InitialDataSeeder
{
    ...
    private async Task SeedUsersAsync(CancellationToken cancellationToken)
    {
        ...
        if (string.IsNullOrWhiteSpace(_settings.SeedAdminPassword))
            throw new InvalidOperationException("Settings:SeedAdminPassword is not configured.");

        var hashedPassword = await _passwordHasher.HashAsync(_settings.SeedAdminPassword, cancellationToken);
