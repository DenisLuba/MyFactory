...
public class PayrollRuleEntity : BaseEntity
{
    ...
    // Constructor
    public PayrollRuleEntity(string name, DateOnly effectiveFrom, decimal premiumPercent, string description)
	{
	...
	public void Update(string name, DateOnly effectiveFrom, decimal premiumPercent, string description)
	{
		Guard.AgainstNullOrWhiteSpace(name, nameof(name));
		Guard.AgainstDefaultDate(effectiveFrom, nameof(effectiveFrom));
		if (premiumPercent < 0)
