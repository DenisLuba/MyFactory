...
public class SalesOrderEntity : AuditableEntity
{
    ...
    public SalesOrderEntity(Guid customerId, DateTime orderDate, Guid createdBy)
    {
    ...
    {
        Guard.AgainstNonPositive(orderNumber, nameof(orderNumber));

        if (OrderNumber > 0 && OrderNumber != orderNumber)
    ...
    public static SalesOrderEntity Create(Guid customerId, DateTime orderDate, Guid createdBy)
    {
        return new SalesOrderEntity(customerId, orderDate, createdBy);
    }
...
public class SalesOrderItemEntity : AuditableEntity
{
    ...
    public void UpdateQtyOrdered(decimal qtyOrdered)
    {
        Guard.AgainstNonPositive(qtyOrdered, nameof(qtyOrdered));
        if (qtyOrdered < QtyAllocated || qtyOrdered < QtyShipped)
            throw new DomainException("QtyOrdered cannot be less than QtyAllocated or QtyShipped.");

        QtyOrdered = qtyOrdered;
        Touch();
    }

    public void UpdateQtyAllocated(decimal qtyAllocated)
    {
        Guard.AgainstNegative(qtyAllocated, nameof(qtyAllocated));
        if (qtyAllocated > QtyOrdered || qtyAllocated < QtyShipped)
            throw new DomainException("QtyAllocated cannot be more than QtyOrdered or less than QtyShipped.");
        
        QtyAllocated = qtyAllocated;
        Touch();
    }

    public void UpdateQtyShipped(decimal qtyShipped)
    {
        Guard.AgainstNegative(qtyShipped, nameof(qtyShipped));
        if (qtyShipped > QtyOrdered || qtyShipped > QtyAllocated)
            throw new DomainException("QtyShipped cannot be more than QtyOrdered or QtyAllocated.");
