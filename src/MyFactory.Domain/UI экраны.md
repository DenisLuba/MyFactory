...
# 4️⃣ Экран: «Производство / Передача в цех»

Это связь с пунктом 4 ТЗ.
...
Я вижу это примерно так. Добавим еще одну кнопку в карточку заказа [Запустить в производство]. И еще во Flyout будет пункт - "Производство". И кнопка Запустить в производство, и пункт "Производство" открывают страницу производственного заказа. Рядом в Tab будет вкладка "Список производственных заказов".
В списке производственных заказов будет список заказов, у которых есть столбцы "Номер" производственного заказа (его нет в таблице ERD только id, но id - слишком длинный, поэтому надо добавить поле с номером, чтобы пользователь как-то различал производственные заказы), "Номер заказа" (номер заказа заказчика, на item которого ссылается этот производственный заказ - не знаю, нужно ли; при коротком нажатии на это поле было бы неплохо открыть страницу соответствующего заказа), "Изделие", "Количество", "Статус" (из enum: "
Enum production_order_status_enum {
  ...
  FINISHED
  CANCELLED
}"). Будет загружаться из PRODUCTION_ORDERS. При длительном нажатии появляются кнопки "Редактировать" и "Удалить". При редактировании можно поменять номер, выбрать изделие из выпадающего списка (например на 5 строчек), как в браузере с реактивным поиском при введении символов, можно поменять статус и количество. Все это прямо в таблице. Если после редактирования ткнуть снаружи строки, то произойдет сохранение и строка снова станет только для просмотра, пока опять не будет долгого нажатия. Можно добавить кнопку "Сохранить", чтобы пользователь сразу понимал, что нужно сделать, чтобы сохранить (а можно не добавлять, посмотрим, как она будет влазить в экран). Также есть кнопка [Добавить], которая открывает страницу производственного заказа.
На странице производственного заказа можно создать производственный заказ. Там есть поля (вертикально расположенные) - номер (он автоматически добавляется - последний заказ увеличенный на 1 - но можно самостоятельно его отредактировать - или нельзя, пока не понимаю, нужно ли его менять самостоятельно), есть поле - заказ (это выпадающий список заказов заказчиков, чтобы привязать его к производственному заказу)