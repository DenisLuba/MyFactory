...
public class FinishedGoodsInventoryTests
{
    ...
    public void ReceiveAndIssue_UpdatesQuantityAndCost()
    {
        ...
        inventory.Receive(10, 5, new DateOnly(2025, 1, 1));
        inventory.Receive(10, 7, new DateOnly(2025, 1, 2));
        inventory.Issue(5, new DateOnly(2025, 1, 3));

        Assert.Equal(15, inventory.Quantity);
        Assert.Equal(new DateOnly(2025, 1, 3), inventory.UpdatedAt);
    ...
    {
        var inventory = new FinishedGoodsInventory(Guid.NewGuid(), Guid.NewGuid());
        inventory.Receive(5, 5, new DateOnly(2025, 1, 1));

        Assert.Throws<DomainException>(() => inventory.Issue(6, new DateOnly(2025, 1, 2)));
    }
}
